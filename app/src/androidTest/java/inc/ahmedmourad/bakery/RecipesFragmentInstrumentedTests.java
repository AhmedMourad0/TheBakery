...
public class RecipesFragmentInstrumentedTests {
	...
	@Test
	public void checkRecipesFragment_startsCorrectly() {

		final Context context = InstrumentationRegistry.getTargetContext();

		onView(withId(R.id.main_title)).check(matches(withText(context.getString(R.string.app_name))));

		onView(withId(R.id.recipes_recycler_view)).check(matches(isDisplayed()));

		onView(withId(R.id.main_add_to_widget)).check(matches(not(isDisplayed())));
		onView(withId(R.id.main_progressbar)).check(matches(not(isDisplayed())));
		onView(withId(R.id.main_back)).check(matches(not(isDisplayed())));
